...
function Boxmodel() {
  ...
  const [mgbot, setmgbot] = useState(
    document?.getElementById("content")?.style?.marginBottom
  );
  const [mglef, setmglef] = useState(
    document?.getElementById("content")?.style?.marginLeft
  );
  const [mgrig, setmgrig] = useState(
    document?.getElementById("content")?.style?.marginRight
  );
  const [brtop, setbrtop] = useState(
    document?.getElementById("content")?.style?.borderTopWidth
  );
  const [brbot, setbrbot] = useState(
    document?.getElementById("content")?.style?.borderBottomWidth
  );
  const [brlef, setbrlef] = useState(
    document?.getElementById("content")?.style?.borderLeftWidth
  );
  const [brrig, setbrrig] = useState(
    document?.getElementById("content")?.style?.borderRightWidth
  );
  const [pdtop, setpdtop] = useState(
    document?.getElementById("content")?.style?.paddingTop
  );
  const [pdbot, setpdbot] = useState(
    document?.getElementById("content")?.style?.paddingBottom
  );
  const [pdlef, setpdlef] = useState(
    document?.getElementById("content")?.style?.paddingLeft
  );
  const [pdrig, setpdrig] = useState(
  ...
  return (
    <Fragment>
      <div className="text-2xl text-center font-medium font-sans">
        Box Model
      </div>
      <section className="min-h-1/2 bg-indigo-950 p-3 py-5 rounded-md w-11/12 ml-auto mr-auto text-white font-medium">
        <div style={{ display: "flex" }}>
          ...
          <div
            id="content"
            className="w-28 m-2 h-28 bg-red-800 rounded-md text-white font-medium text-sm text-center "
            style={{
              margin: mgcontent,
              padding: pdcontent,
              borderStyle: "solid",
      ...
      <section className="flex flex-col md:flex-row w-11/12 ml-auto mr-auto justify-around">
        <div>
          ...
          <div className="flex">
            <div className="mt-auto mb-auto">
              ...
              <input
                type="text"
                value={mgcontent}
                onChange={(e) => setMgcontent(e.target.value)}
                className="border rounded-lg h-6"
              />{" "}
            </div>
            <div className="flex">
              <div
          ...
          <div className="flex">
            <div className="mt-auto mb-auto">
              ...
              <input
                type="text"
                value={pdcontent}
                onChange={(e) => setPdcontent(e.target.value)}
                className="border rounded-lg h-6"
              />{" "}
            </div>
            <div className="flex">
          ...
          <div>
            <div>Content</div>
            <div className="mt-auto mb-auto">
              size : <input type="text" className="border rounded-lg h-6" />{" "}
            </div>
          </div>
        </div>
        <div
          className="flex flex-col ml-auto mr-auto m-2 border-dashed border-05 border-black"
          style={{ width: "350px", height: "350px", background: "#b08354" }}
        >
          <div className="pl-2 text-sm">Margin</div>
          <div className="text-center ">{mgtop}</div>
          <div className="flex">
            <div className="mt-auto mb-auto mr-auto ml-auto text-center">
              {mglef}
            </div>
            <div
              className="flex flex-col ml-auto mr-auto border-05 border-black"
              style={{ width: "250px", height: "250px", background: "#e4c482" }}
            >
              ...
              <div className="flex">
                ...
                <div
                  className="flex flex-col mr-auto ml-auto border-dashed border-05 border-black"
                  style={{
                    width: "150px",
                    height: "150px",
                    background: "#b8c480",
                ...
                >
                  ...
                  <div className="flex">
                    ...
                    <div
                      className="mr-auto ml-auto border-05 border-black"
                      style={{
                        width: "80px",
                        height: "50px",
                        background: "#88b2bd",
